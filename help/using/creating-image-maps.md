...
1. Draw a rectangular or polygonal (many-sided) Image Map:
   ...
   * **Polygonal map**: Select the Polygon Image Map tool and select points on the perimeter of the area of the image you want to enclose. Use the polygon density slider to vary the point density in the polygon. The original density is remembered if you select other maps. If any point is added, deleted, or moved in the polygon, the original density is lost. The slider is reset to its maximum value.
...
If your image or eCatalog page includes more than one Image Map and the maps overlap, you can determine how the maps overlap. To do so, change the order of maps on the Image Map list. Drag their names higher or lower on the list. How high a name is on the list determines whether its Image Map overlaps other Image Maps.
...
1. On the Map Editor screen (images and SpinSets) or the Map Pages tab of the eCatalog screen (eCatalogs), select the **[!UICONTROL Show]** menu and select **[!UICONTROL Other Actions]**.
...
1. Select **[!UICONTROL OK]**. (Adobe Dynamic Media Classic completes the URL using the Image Map URL template.)
   ...
   >[!NOTE]
   >
   >You do not need to set appearance options in Adobe InDesign. You can specify the appearance in Adobe Dynamic Media Classic.
...
>[!NOTE]
>
>You do not need to set appearance options in Adobe Acrobat. You can specify the appearance in Adobe Dynamic Media Classic.
