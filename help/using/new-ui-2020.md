...
Adobe Flash Player was a Web browser plug-in that allowed Web browsers to use content developed on the Adobe Flash Platform. The Web user interface of Adobe Dynamic Media Classic (currently labeled [!DNL Scene7 Publishing System] or [!DNL SPS] in the product) required the Adobe Flash Player. When Adobe Flash is deprecated on December 31, 2020, Adobe Dynamic Media Classic customers are no longer able to log into the Web user interface. Because of this change, Adobe has provided customers with a desktop app that replaces the browser experience.
+++

+++**_How do I access the new desktop app?_**
The new desktop app is available as a `.dmg` installer for macOS. `.exe` installer for Windows&reg;.
...
>[!IMPORTANT]
>
>Only one instance of the desktop app can be installed and active at the same time on a given computer. However, there is no restriction on the number of installations you can have across computers.
...
No. There is no impact to automation scripts. The new desktop app functions similarly to the browser-based experience.
+++
...
Some companies do not permit you to download and install software and apps onto your system without approval. To obtain permission to access the new Adobe Dynamic Media Classic desktop app, coordinate with your IT team in advance. Remember that after December 31, 2020, the browser version is deprecated. It is important that you download the new desktop app before the deadline.
+++

+++**_Can multiple instances of the new desktop app be open at the same time?_**
No. The Adobe&reg; AIR&reg; technology on which the new Adobe Dynamic Media Classic desktop app is built restricts a user from having multiple instances of the app open at a given time.
+++
...
If Adobe releases a new version of the desktop app, customers must download and install it (replace the existing app in **[!UICONTROL Applications]**). You are notified of the new release through your Adobe Account Team and an in-app upgrade notification mechanism that notifies users of an upgrade.
+++
...
Adobe Dynamic Media Classic is a comprehensive solution designed to support your rich media workflows. To ensure you use all the capabilities, explore these practical resources:
...
* [Adobe Blog Posts](https://blog.adobe.com/)
